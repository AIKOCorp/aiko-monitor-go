...
func main() {
	...
	handler := func(ctx *fasthttp.RequestCtx) {
		...
		case path == "/" && method == fasthttp.MethodGet:
			writeHTML(ctx, `<html><body><h1>FastAPI E-commerce API</h1><p>Welcome to our store!</p></body></html>`)
		...
		case strings.HasPrefix(path, "/auth/verify/") && method == fasthttp.MethodGet:
			...
			if token == "abc123" {
				writeHTML(ctx, `<html><body><h1>Email Verified</h1><p>Your email has been verified successfully!</p></body></html>`)
			} else {
				writeHTML(ctx, `<html><body><h1>Invalid Token</h1><p>Token not found or expired</p></body></html>`)
			}
...
func parseJSON(ctx *fasthttp.RequestCtx, dst any) bool {
...
func writeHTML(ctx *fasthttp.RequestCtx, body string) {
