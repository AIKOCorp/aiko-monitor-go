...
func TestParseJSONBodyVariants(t *testing.T) {
	...
	if !ok || m["a"].(float64) != 1 {
		t.Fatalf("expected parsed JSON map, got %#v", obj)
	}

	raw := aiko.ParseJSONBody([]byte("not-json"))
	if raw.(string) != "not-json" {
		t.Fatalf("expected passthrough string, got %#v", raw)
	}
}

func TestDecodeResponseBodyDecodesCompressedPayloads(t *testing.T) {
	...
	decoded := aiko.DecodeResponseBody(gzBuf.Bytes(), map[string]string{
		"content-type":     "application/json",
		"content-encoding": "gzip",
	})
	gzMap, ok := decoded.(map[string]any)
	if !ok || gzMap["ok"].(bool) != true {
		t.Fatalf("expected gzip JSON map, got %#v", decoded)
	}

	var deflateBuf bytes.Buffer
	zl := zlib.NewWriter(&deflateBuf)
	if _, err := zl.Write(payload); err != nil {
	...
	zlDecoded := aiko.DecodeResponseBody(deflateBuf.Bytes(), map[string]string{
		"content-type":     "application/json",
		"content-encoding": "deflate",
	})
	zlMap, ok := zlDecoded.(map[string]any)
	if !ok || zlMap["ok"].(bool) != true {
		t.Fatalf("expected deflate JSON map, got %#v", zlDecoded)
	}
...
func TestGzipEventRoundTrip(t *testing.T) {
	...
	defer func() {
		if cerr := zr.Close(); cerr != nil {
			t.Fatalf("close gzip reader: %v", cerr)
		}
	}()
